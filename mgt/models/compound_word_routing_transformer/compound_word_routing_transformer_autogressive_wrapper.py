...
class AutoregressiveWrapper(nn.Module):
    def __init__(self, net, ignore_index=None, pad_value=0):
        ...
        assert isinstance(net,
                          RoutingTransformerLM), 'generative trainer wrapper can only accept RoutingTransformerLM class'
    ...
    def forward(self, x, return_loss=False, randomly_truncate_sequence=False, **kwargs):
        ...
        if m is not None:
            assert m.shape == x.shape[
                              0:2], 'input mask must be the same shape as the input of the auto-regressive wrapper to automatically handle'
